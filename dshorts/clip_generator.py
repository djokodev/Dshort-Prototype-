...
class ClipGenerator:
    ...
    def get_video_duration(self):
        ...
        try:
            ...
            logger.error(
                f"Erreur lors de l'obtention de la durée de la vidéo: {str(e)}"
            )
            return 0

    def generate_candidate_segments(self, step_size=5, window_sizes=None):
        """
        ...
        # Utilisez une gamme plus variée de tailles de fenêtres, optimisée
        window_sizes = window_sizes or [15, 25, 35, 45]
        segments = []

        # Calculer le nombre approximatif de segments requis pour couvrir toute la vidéo
        target_segments = (
            int(duration / min(window_sizes)) * 2
        )  # Facteur réduit pour plus de performance

        # Ajuster le pas pour obtenir un nombre raisonnable de segments
        if target_segments > 500:  # Réduit de 1000 à 500 pour plus de performance
            # Limiter le nombre de segments pour des raisons de performance
            step_size = max(step_size, int(duration / (500 / len(window_sizes))))

        print(
            f"Génération de segments avec pas de {step_size}s et fenêtres {window_sizes}s"
        )
        ...
        for window_size in window_sizes:
            ...
            # Générer des segments espacés régulièrement, optimisé
    ...
    def filter_overlapping_segments(self, scored_segments, min_separation=15):
        ...
        for start, end, score in top_candidates:
            ...
            footprint_overlap = np.sum(temporal_footprint[segment_range]) / len(
                segment_range
            )
        ...
        if len(selected) < 3:  # Si on n'a pas assez de segments
            ...
            for start, end, score in remaining:
                ...
                for sel_start, sel_end, _ in selected:
                    ...
                    if (
                        abs(midpoint - sel_midpoint) < min_separation / 2
                    ):  # Relâché pour la seconde passe
                        too_close = True
                        break
    ...
    def find_optimal_endpoint(
        self, audio_analyzer, start_time, end_time, max_extend=5.0
    ):
        """
        Trouve un point de fin optimal pour éviter de couper en pleine phrase

        Args:
            audio_analyzer: Instance d'AudioAnalyzer pour détecter les silences
            start_time (float): Temps de début du clip en secondes
            end_time (float): Temps de fin initial du clip en secondes
            max_extend (float): Extension maximale autorisée en secondes

        Returns:
            float: Nouveau temps de fin optimisé
        """
        try:
            # Récupérer les silences détectés
            silences = audio_analyzer.detect_silences(
                threshold_db=-35, min_silence_duration=0.3
            )

            # Chercher un silence dans les N secondes après la fin prévue
            search_end = min(end_time + max_extend, self.get_video_duration())

            best_silence = None
            min_distance = max_extend + 1  # Plus grand que max_extend au départ

            for silence_start, silence_end in silences:
                # Chercher un silence qui commence après le temps de fin initial
                # mais avant la limite d'extension
                if end_time <= silence_start < search_end:
                    distance = silence_start - end_time

                    if distance < min_distance:
                        min_distance = distance
                        # Utiliser le début du silence comme point de coupure
                        best_silence = silence_start

            if best_silence is not None:
                logger.info(
                    f"Point de fin optimisé: {end_time:.2f}s → {best_silence:.2f}s (+{min_distance:.2f}s)"
                )
                return best_silence

            # Si aucun silence trouvé, chercher un point de faible énergie audio
            if hasattr(audio_analyzer, "energy") and audio_analyzer.energy is not None:
                hop_length = 512
                ...
                if sr is not None and sr > 0:
                    ...
                    search_end_frame = int(search_end * sr / hop_length)

                    # Limiter aux indices valides
                    end_frame = max(0, min(end_frame, len(audio_analyzer.energy) - 1))
                    search_end_frame = max(
                        0, min(search_end_frame, len(audio_analyzer.energy) - 1)
                    )

                    if end_frame < search_end_frame:
                        # Extraire l'énergie dans la zone de recherche
                        search_energy = audio_analyzer.energy[
                            end_frame:search_end_frame
                        ]

                        if len(search_energy) > 0:
                            # Trouver le point de plus faible énergie
                            min_energy_idx = np.argmin(search_energy)
                            best_time = (end_frame + min_energy_idx) * hop_length / sr

                            logger.info(
                                f"Point de faible énergie trouvé: {end_time:.2f}s → {best_time:.2f}s"
                            )
                            return best_time

            # Si aucune optimisation trouvée, retourner le temps initial
            return end_time

        except Exception as e:
            logger.warning(
                f"Erreur lors de la recherche du point de fin optimal: {str(e)}"
            )
            return end_time

    def extract_clip(self, start_time, end_time, audio_analyzer=None, output_path=None):
        """
        Extraire un clip de la vidéo originale avec optimisation des points de coupure

        Args:
            start_time (float): Temps de début en secondes
            end_time (float): Temps de fin en secondes
            audio_analyzer (AudioAnalyzer, optional): Instance d'AudioAnalyzer pour optimiser les coupures
        ...
        try:
            ...
            # Optimiser le point de fin si un analyseur audio est fourni
            optimized_end_time = end_time
            if audio_analyzer is not None:
                optimized_end_time = self.find_optimal_endpoint(
                    audio_analyzer, start_time, end_time
                )
            ...
            with VideoFileClip(self.video_path) as video:
                ...
                optimized_end_time = min(video.duration, optimized_end_time)

                if optimized_end_time <= start_time:
                    raise ValueError("Temps de fin inférieur ou égal au temps de début")

                # Créer le sous-clip et sauvegarder
                subclip = video.subclip(start_time, optimized_end_time)
                subclip.write_videofile(
                    ...
                    remove_temp=True,
                    preset="faster",  # Accélérer l'encodage (au lieu de 'medium')
                    threads=4,  # Plus de threads pour accélérer l'encodage
                    verbose=False,
                    logger=None,
                )
    ...
    def _process_clip(self, clip_info):
        """
        Fonction helper pour le traitement parallèle des clips

        Args:
            clip_info: Tuple (index, start, end, score)

        Returns:
            tuple: (clip_path, start_time, end_time, score) ou None en cas d'échec
        """
        i, start, end, score, audio_analyzer = clip_info
        ...
        # Nommage standardisé
        filename = (
            f"short_{i+1}_score_{int(score*100)}_time_{start_rounded}_{end_rounded}.mp4"
        )
        output_path = os.path.join(self.output_dir, filename)

        # Extraire le clip avec optimisation des points de coupure
        clip_path = self.extract_clip(
            start_rounded, end_rounded, audio_analyzer, output_path
        )

        if clip_path:
            return (clip_path, start_rounded, end_rounded, score)
        return None

    def generate_shorts(
        self, video_analyzer, audio_analyzer, text_analyzer, num_shorts=5
    ):
        """
        ...
        try:
            ...
            # Utiliser du multithreading pour évaluer les segments en parallèle
            def evaluate_segment(segment):
                ...
                # Combiner les scores
                combined_score = self.score_segment(
                    start, end, video_score, audio_score, text_score
                )

                if combined_score > 0:
                    return (start, end, combined_score, region_idx)
                return None

            # Utiliser une pool de threads pour paralléliser l'évaluation des segments
            with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
                results_futures = list(
                    executor.map(evaluate_segment, self.candidate_segments)
                )

            # Filtrer les résultats None
            scored_segments = [
                result for result in results_futures if result is not None
            ]

            print(
                f"Segments candidats évalués: {len(scored_segments)} segments valides"
            )
            ...
            for region_idx in range(len(video_regions)):
                ...
                region_segments = [
                    (s, e, score)
                    for s, e, score, r_idx in scored_segments
                    if r_idx == region_idx
                ]
                # Trier par score
            ...
            for segment in all_segments:
                if segment not in combined_selections:
                    ...
                    if (
                        len(combined_selections) >= num_shorts * 2
                    ):  # Prendre 2x plus pour le filtrage
                        break

            # Filtrer les segments qui se chevauchent trop
            print("Filtrage des segments qui se chevauchent...")
            min_separation = max(
                15, video_duration / (num_shorts * 2)
            )  # Assurer une bonne séparation
            filtered_segments = self.filter_overlapping_segments(
                combined_selections, min_separation
            )
            ...
            # Préparation des arguments pour l'extraction parallèle
            clip_args = [
                (i, start, end, score, audio_analyzer)
                for i, (start, end, score) in enumerate(top_segments)
            ]

            # Utiliser un threadpool pour extraire les clips en parallèle
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
                clip_results = list(executor.map(self._process_clip, clip_args))

            # Filtrer les résultats None
            results = [result for result in clip_results if result is not None]

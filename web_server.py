...
def process_video(task_id, video_path, num_shorts=3, min_duration=10, max_duration=60, whisper_model='base', language='fr'):
    """
    ...
    try:
        ...
        # 3. Transcription
        tasks[task_id]['message'] = 'Transcription en cours (peut prendre du temps)...'
        text_analyzer = TextAnalyzer(video_path, model_size=whisper_model, language=language, temp_dir=TEMP_FOLDER)
        ...
        tasks[task_id]['message'] = f'{len(generated_shorts)} shorts générés avec succès'
...
class DshortsRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/api/upload':
            ...
            whisper_model = form.getvalue('whisper_model', 'base')
            ...
            thread = threading.Thread(
                ...
                args=(task_id, filepath, num_shorts, min_duration, max_duration, whisper_model, language)
            )
            thread.daemon = True
            thread.start()
